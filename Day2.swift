...
let correctPasswords = input.filter {
    
    let components = $0.components(separatedBy: CharacterSet(charactersIn: "-: "))
    
    guard let firstNumber = Int(components[0]),
          let secondNumber = Int(components[1]) else { return false }
    
    let range = firstNumber...secondNumber
    let requiredLetter = components[2]
    let password = components[4]
    
    return range.contains(password.components(separatedBy: requiredLetter).count)
}
...
let newRulePasswords = input.filter {
    ...
    let requiredLetter = Character(components[2])
    let password = components[4]
