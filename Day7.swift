...
let input = inputDay7.trimmingCharacters(in: .whitespacesAndNewlines).components(separatedBy: .newlines).map { $0.components(separatedBy: " ") }

class Bag: Hashable, Equatable {
    
    let color: String!
    var contentRule: [Bag : Int]?
    ...
    func canHoldShiny() -> Bool {
        contentRule?[Bag(color: "shinygold")] != nil
    }
    ...
    func shinyCount() -> Int {
        ...
        if contentRule != nil {
            if canHoldShiny() {
                count += contentRule![Bag(color: "shinygold")] ?? 0
            }
            for key in contentRule!.keys {
                count += key.shinyCount()
            }
        }
        return count
    }
    ...
    func bagCount() -> Int {
        ...
        if contentRule != nil {
            for key in contentRule!.keys {
                if key.contentRule != nil {
                    for _ in 0..<contentRule![key]! {
                        count += key.bagCount()
                    }
                }
                count += contentRule![key]!
            }
        }
        return count
    }
...
for bagRule in input {
    
    let newBag = Bag(color: bagRule[0]+bagRule[1])
    var contentRules:  [Bag : Int]? = nil
    
    var x = 4
    if bagRule[x] != "no" {
        contentRules = [Bag : Int]()
        while x < bagRule.count {
            
            let contentBag = Bag(color: bagRule[x+1]+bagRule[x+2])
            
            if bags.contains(contentBag) {
                let bag = bags.first(where: {$0 == contentBag })!
                
                contentRules![bag] = Int(bagRule[x])
            } else {
                bags.append(contentBag)
                contentRules![contentBag] = Int(bagRule[x])
            }
    ...
    if bags.contains(newBag) {
        ...
        let bag = bags.first(where: {$0 == newBag })!
        bag.contentRule = contentRules
        
    } else {
        newBag.contentRule = contentRules
        bags.append(newBag)
    }
}
...
print(
    "Part 1: ",
    bags.filter { $0.shinyCount() > 0 }.count
)

//MARK: Day 7 Part 2 -

let bag = bags.first(where: {$0 == Bag(color: "shinygold") })

print("Part 2: ",bag!.bagCount())


public let inputDay7 = """
